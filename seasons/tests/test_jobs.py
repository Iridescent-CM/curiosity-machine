...
from ..middleware import get_season_config, SeasonConfig
...
def test_record_season_creates_season():
    ...
    _record_season(get_season_config(), UserFactory().id, now())
...
def test_record_season_connects_user_and_season():
    ...
    user = UserFactory()
    _record_season(get_season_config(), user.id, now())
...
def test_record_season_only_records_once_per_season():
    user = UserFactory()
    season_1 = SeasonConfig(now(), now(), "season 1")
    season_2 = SeasonConfig(now(), now(), "season 2")
...
def test_record_season_keeps_first_record():
    ...
    _record_season(get_season_config(), user.id, time_1)
    _record_season(get_season_config(), user.id, time_2)
