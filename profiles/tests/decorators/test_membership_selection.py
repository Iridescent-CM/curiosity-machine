...
def test_membership_selection_one_membership(rf):
    request = rf.get('/home')
    memberships = [{"id": 5, "display_name": "name"}]
    request.session = mock.MagicMock()

    ms = MembershipSelection(request, memberships)
    assert ms.count == 1
    assert ms.all == [{"id": 5, "display_name": "name"}]
    assert ms.selected == {"id": 5, "display_name": "name"}
...
def test_membership_selection_many_memberships(rf):
    request = rf.get('/home')
    memberships = [{"id": 5, "display_name": "name"}, {"id": 6, "display_name": "name"}]
    request.session = mock.MagicMock()

    ms = MembershipSelection(request, memberships)
    assert ms.count == 2
    assert ms.all == [{"id": 5, "display_name": "name"}, {"id": 6, "display_name": "name"}]
    assert ms.selected == {"id": 5, "display_name": "name"}
