...
from curiositymachine.decorators import feature_flag, educator_only, student_only, educator_or_student_only
...
@feature_flag('enable_groups')
@educator_or_student_only
def group(request, group_id): 
    invite_form = GroupInviteForm(data=request.POST)
    group = get_object_or_404(Group, id=group_id)
    owners = group.owners()
    return render(request, 'group.html', {'group': group, 'invite_form': invite_form, 'owners': owners})
